...
In this analysis we are using  a series of rules to determine whether simulated hydralics are suitable for the different life history phases of the Gooddingg's Black willow.  The hydralics are simulated for the Los Angeles River using different waster water reuse scenarios. We start by loading the hydralic data.  The hydrulic data shows hourly data for each cross section of interest and the variables streamflow, water depth, velocity, shear stress, stream power, and water table depth. For now, we will just generate fake data.
...
We also load the models or the thresholds used to predict some measure of suitability based on the hydraulics. the models are continuous relatiomships between the hydraulic variable and percent mortality.  The thresholds show the minimum or maximum values that certain hydraulic need to reach to create suitabilty condiitons. Specifics are discussed in each life history stage section below.
...
First, we determine the suitabliity for each node, bank, and year to support black willow germination.  Germination suitalbity is determined using inundation duration and magnitude. 

For inundation duration we will use data from Nakai, A., & Kisanuki, H. (2007). Effect of inundation duration on Salix gracilistyla density and size on a gravel bar. Journal of Forest Research, 12(5), 365–370. https://doi.org/10.1007/s10310-007-0027-7. They found that inundation durations longer than 280 days or less than 85 days led to low or zero tree density 1-4 years later, suggesting that germination was unsuccessful. We will use 5cm as a depth to represent innundation, anything less than this would be unreliable output from the HEC-RAS model.
